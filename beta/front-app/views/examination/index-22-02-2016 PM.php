...
<div class="mcqDiv">
    <div class="wrap clear">
        ...
        <div class="innerDiv">
        <h3><?php echo 'WELCOME '.$student_name;?></h3>
        <div class="secondInnerDiv">
            <div class="topOptions"><span class="preview">PREVIEW</span><span class="quit">QUIT</span></div>
...
Q.<?php echo ($k+1)?> <img src="<?php echo (isset($v['title']) && $v['title']!='' && file_exists(FILE_UPLOAD_ABSOLUTE_PATH."question/".$v['image']) ? BACKEND_IMAGE_PATH."question/".$v['image'] : BACKEND_IMAGE_PATH.'no_image.png');?>" alt="No Image" height="70" width="90"/>
<h4 class="questionH4"><?php echo stripslashes($v['title']);?></h4>
                ...
                <div>
                    <div class="bottomOptions">
                        <?php if(COUNT($question_list) > 1){?>
                            <span id="prev_question" class="navigation" style="display: none;">PREV</span>
                ...
                <div class="terms_condition">
                    ...
                    <div class="buttons">
                        <input type="button" id="i_accept" class="i_accept" value="I Agree"/>
...
<script>
    $(document).on("click", '.navigation', function(event) {
        ...
        if(click_id == 'next_question' || click_id == 'save_next')
        {
            ...
            if($('#question_'+current_qsn).next('.question_div').attr('id').replace('question_','') == $('#last_question').val())
            {
               $('#next_question,#save_next').css('display','none');
               $('#save_finish').css('display','block');
            }
            $('#prev_question').css('display','block');
        }
        else if(click_id == 'prev_question')
        {
            $('#save_finish').css('display','none');
            ...
            //if($('#question_'+current_qsn).next('.question_div').attr('id').replace('question_','') == $('#first_question').val())
            if(current_qsn == $('#first_question').val())
            {
               $('#prev_question').css('display','none');              
            }
            
            if($('#current_question').val() == $('#first_question').val())
               $('#prev_question').css('display','none');                      
        }
        else if(click_id == 'save_finish')
        {
            var r = confirm("This will submit the exam.");
            if (r == true)
                $('#exam_form').submit();
        }
        return true;
    ...
    $(document).on("click", '.questionTitle', function(event) {
        ...
        {
            $('#prev_question,#save_finish').css('display','none');
        ...
        else if(question_id == $('#last_question').val())
        {
            $('#next_question,#save_next').css('display','none');
            $('#prev_question,#save_finish').css('display','block');
        }
        else
        {
            $('#save_finish').css('display','none');
