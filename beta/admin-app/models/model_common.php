...
class model_common extends CI_Model
{
	...
	*/
	...
	{
		$sql = "SELECT qm.id, qm.title AS question, qm.image, qm.question_type, qm.group AS subject_group, qm.question_order AS question_order, qm.is_active AS question_status, am.title AS answer, am.is_correct AS is_correct_answer, am.is_active AS answer_status, sm.id AS subject_id, sm.title AS subject_title			
			FROM ".QUESTION." AS qm
			LEFT JOIN ".ANSWER." AS am ON qm.id = am.question_id
			LEFT JOIN ".SUBJECT." AS sm ON qm.subject_id = sm.id
			WHERE qm.id='".$id."'
			ORDER BY am.is_correct";
		$rs	= $this->db->query($sql);
		$rec	= false;
