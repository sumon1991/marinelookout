...
//........................	
	...
	public function category_list(){
		...
		$categoryData = [];
		$catList = $this->model_blog->getcategoryList($config,$start);
		foreach ($catList as $key => $cat) {
			$categoryData[$key]['catData'] = $cat;
			if($cat['parent_id']){
				$parenData = $this->model_blog->getParent($cat['parent_id']);
				$categoryData[$key]['parent_cat'] = $parenData['name'];
			} else {
				$categoryData[$key]['parent_cat'] = 'It is parent';
			}
		}
		$this->data['categoryList'] = $categoryData;
		// pr($this->data['categoryList']);
		// die;
		$this->data['startRecord'] 		= $start;
	...
	public function category_add(){
		...
		if($this->input->post('action') == 'process'){
			...
			$slug = trim(strtolower(preg_replace('/\s+/', '_', $this->input->post('slug'))));
