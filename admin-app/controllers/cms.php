...
class Cms extends MY_Controller{
    ...
    public function shortdescription()
    {
		...
		if($this->input->get_post('action') == 'edit') {
			$cms_content	= 	trim($this->input->post('cdetails'));
			$cms_title		= 	trim($this->input->post('ctitle'));
			$updArr  =  array(
						'description_header'=>$cms_title,
						'short_description' => $cms_content
					);
			$idArr		= 	array( 'id' => 1 );			    
			$ret		=	$this->model_basic->updateIntoTable('ep_short_about', $idArr, $updArr);
			if(isset($ret))
			{
			    $this->nsession->set_userdata('succmsg', "Description updated successfully.");
			}
			else
			{
			    $this->nsession->set_userdata('errmsg', "Unable to update. Please try again later.");
			}						
			redirect(BACKEND_URL."cms/shortdescription/");
			return true;			    
		}
	
		//........................	
	    $this->data['controller']='cms';
	    $detailedData = $this->model_basic->getShortDescription();
	    $this->data['short_description'] =	trim($detailedData['short_description']);
	    $this->data['description_header'] = trim($detailedData['description_header']);
	    $this->data['succmsg'] = $this->nsession->userdata('succmsg');
		$this->data['errmsg'] = $this->nsession->userdata('errmsg');		
		$this->nsession->set_userdata('succmsg', "");		
		$this->nsession->set_userdata('errmsg', "");
		...
		$this->templatelayout->get_leftmenu();
		$this->templatelayout->get_footer();
	    $this->elements['middle']='cms/short_description';			
	    $this->elements_data['middle'] = $this->data;			    
	    $this->layout->setLayout('layout');
	    $this->layout->multiple_view($this->elements,$this->elements_data);      
